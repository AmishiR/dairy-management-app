...
type Line = { order_item_id: string; allocations: Allocation[] }

// Confirms a dispatch: the client sends the per-line batch allocation as a
// JSON blob, this hands it to the confirm_dispatch() Postgres function which
// atomically creates the delivery + delivery_items + dispatch_allocations and
// lets the existing delivery triggers deduct stock and advance the order.
export async function confirmDispatch(formData: FormData) {
  ...
  // Drop lines with nothing allocated so an untouched product doesn't create
  // an empty delivery line.
  const payload = lines
    .map((l) => ({
      order_item_id: l.order_item_id,
      allocations: (l.allocations ?? []).filter((a) => a.batch_id && a.quantity > 0),
    }))
  ...
  if (payload.length === 0) {
    redirect(`/staff/dispatch/${order_id}?error=${encodeURIComponent('Allocate at least one batch to a product.')}`)
  }
